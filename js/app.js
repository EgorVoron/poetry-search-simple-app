import { fetchRandomPoem, fetchPoemById, fetchSimilarPoems, searchPoems } from './api.js';
...
async function loadSimilarPoems(poemId) {
    ...
    try {
        const result = await fetchSimilarPoems(poemId);
        if (result.rateLimited) {
            similarContainer.innerHTML = '<div class="loading">Слишком частые запросы</div>';
            return;
        }
        displaySimilarPoems(result.poems);
    } catch (error) {
...
async function searchPoemsByText(queryText) {
    try {
        const result = await searchPoems(queryText);
        if (result.rateLimited) {
